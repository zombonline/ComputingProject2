...
function validateArrivalTime(value){
    const checks = [
        ...
        {condition: parseInt(value) >= 0 && parseInt(value) <= 2359, errorMessage: "Not a valid time"}
    ];
...
function validateCommuteName(value){
    const checks = [
        ...
        {condition: !REG_SPECIAL.test(value), errorMessage: "Name contains special characters" },
    ];
    return performValidation(value, checks);
}
/**
...
module.exports = {validateArrivalTime, validateCommuteName, validateAverageDuration, validateLatLong};